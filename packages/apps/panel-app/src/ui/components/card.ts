...
export function $CardRenderer(
  ...
  ownerName: string,
  bank: BankInfo,
  layer?: () => TemplateResult,
) {
  const bColor = bank.color;
  ...
  return html`
    <div class="w-full h-56 bg-darkSurface rounded-2xl shadow-xl relative overflow-hidden">
      ...
      <div class="absolute inset-0 z-[5] flex flex-col pt-3 pb-6 px-6">
        <div class="size-20 flex items-center justify-center *:h-full *:w-full *:object-contain">${bank.image}</div>
        ...
        <div class="flex justify-between items-center text-bodyMedium text-outline pt-1 mt-1" dir="ltr">
          <span class="opacity-85 cursor-pointer">IR${iban}</span>
        ...
        <div class="text-bodyMedium text-outline opacity-85">${ownerName}</div>
      </div>
    </div>
  `;
}
